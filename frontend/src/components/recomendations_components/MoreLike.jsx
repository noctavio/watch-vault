        ...
        const fetchMovies = async () => {
            ...
            try {
                const watchlistRes = await fetch(
                    `http://localhost:8080/api/watchlist/${user.watchlistId}`
                );
                ...
                if (items.length === 0) {
                    setAlert({ type: "info", message: "Your watchlist is empty! Add some movies to get recommendations here." });
                    setLoading(false);
                    return;
                }
                ...
                for (const movie of shuffledItems) {
                    const res = await fetch(
                        `http://localhost:8080/api/recommendations/movie/${movie.id}`
                    );
                ...
                const fallbackRes = await fetch(
                    "http://localhost:8080/api/recommendations/random/movie"
                );
                const fallbackData = await fallbackRes.json();
                if (!fallbackRes.ok)
                    throw new Error(fallbackData.error || "Failed to fetch random movies.");
                const randomMovies = (fallbackData.movies || fallbackData.results || [])
